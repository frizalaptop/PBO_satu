...
class Root(Tk):
    def __init__(self):
        ...
        self.result.grid(row=3, column=0,columnspan=2, pady=20)
    ...
    def btn_clicked(self):
        ...
        if self.language == 'Jerman':
            hasil = penerjemah.translate(self.txtSrc.get(), src='id', dest='de') 
            self.result.delete(0, END)
            self.result.insert(0, hasil.text)
        elif self.language == 'Spanyol':
            hasil = penerjemah.translate(self.txtSrc.get(), src='id', dest='es') 
            self.result.delete(0, END)
            self.result.insert(0, hasil.text)
        else:
            hasil = penerjemah.translate(self.txtSrc.get(), src='id', dest='mn') 
            self.result.delete(0, END)
            self.result.insert(0, hasil.text)
